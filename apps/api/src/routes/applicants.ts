...
router.post('/upload', upload.single('file'), async (req, res, next) => {
  try {
    ...
    // create applicant record (Airtable or DB)
    const applicant = await airtableSvc.createApplicant({ name, email, resume_url: storedUrl, resume_text: parsed.text, external_id: applicantId })
...
router.get('/:id/status', async (req, res, next) => {
  try {
    ...
    const matches = await matchingSvc.getCachedMatchesForApplicant(id)
    res.json({ status: applicant.status, matches, notes: applicant.notes || null })
  } catch (err) {
    next(err)
  }
})
